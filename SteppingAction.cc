...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
 //get volume of the current step
  ...
  //entrance and exit points in/from target for primary and secondaries
  G4ThreeVector pointEnPrimT, pointExPrimT, pointExSecT;
  ...
  if (track->GetTrackID() == 1)  {
    ...
    if (volume == fDetectorconstruction->GetTarget())   {
        ...
        if (pre->GetStepStatus() == fGeomBoundary)   {
    	   ...
    	   pointEnPrimT = pre->GetPosition();
    	   
    	   fEventaction->AddPointEntranceT(pointEnPrimT);
	...
	if (post->GetStepStatus() == fGeomBoundary)   {
	   ...
	   fEventaction->AddPointExitT(pointExPrimT);
